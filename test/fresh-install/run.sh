...
#
# Usage: npm run test:fresh-install -- v0.5.0                 (specific tag)
#        npm run test:fresh-install -- v0.5.0 npm             (tag + fixture)
#        npm run test:fresh-install -- v0.5.0 pnpm|yarn|bun   (other PM)
#        npm run test:fresh-install                           (latest tag)
#
...
[ -z "$TAG" ] && { echo "✗ no tag — pass one (npm run test:fresh-install -- v0.5.0) or cut a tag first."; exit 2; }

FIXTURE="${2:-npm}"  # default fixture: npm
CONSUMER_REPO="${CONSUMER_REPO:-https://github.com/csrinaldi/samples-of-html5.git}"

echo "▶ TOTAL fresh-install | tag=${TAG} | fixture=${FIXTURE} | image=${IMAGE}"
