...
>
> ```bash
> npm i -D "git+https://github.com/csrinaldi/brain.git#v1.5.0"
> ```
...
```bash
npm run brain:upgrade -- v1.5.0             # install a newer tag, copy managed paths
npm run brain:upgrade -- v1.5.0 --dry-run   # preview what would change
```
...
| `npm run brain:memory:pull` | **Cross-machine sync**: safe pull — discards regenerable manifest churn, runs `git pull`, then imports `.memory/` into local engram. Use this instead of raw `git pull` when `.memory/manifest.json` is dirty (i.e. after `brain:memory:share` ran locally). Raw `git pull` may abort with "your local changes would be overwritten" when the manifest is uncommitted. |
