...
npm i -D "git+https://github.com/csrinaldi/brain.git#v0.9.1"
...
>
> ```bash
> pnpm add -D "git+https://github.com/csrinaldi/brain.git#v0.9.1"
> pnpm run brain:upgrade -- v0.9.1     # brain:upgrade installs via your detected PM
> pnpm run brain:env:init
> pnpm run brain:day:start
> ```
>
> (yarn: `yarn add … && yarn brain:upgrade -- v0.9.1`; bun: `bun add -d … && bun run brain:upgrade`.)
...
```bash
npm run brain:upgrade -- v0.9.1             # install a newer tag, copy managed paths
npm run brain:upgrade -- v0.9.1 --dry-run   # preview what would change
```
