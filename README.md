...
npm i -D "git+https://github.com/csrinaldi/brain.git#v0.9.5"
...
>
> ```bash
> pnpm add -D "git+https://github.com/csrinaldi/brain.git#v0.9.5"
> pnpm run brain:upgrade -- v0.9.5     # brain:upgrade installs via your detected PM
> pnpm run brain:env:init
> pnpm run brain:day:start
> ```
>
> (yarn: `yarn add … && yarn brain:upgrade -- v0.9.5`; bun: `bun add -d … && bun run brain:upgrade`.)
...
```bash
npm run brain:upgrade -- v0.9.5             # install a newer tag, copy managed paths
npm run brain:upgrade -- v0.9.5 --dry-run   # preview what would change
```
