...
| **standard** | Small team (n≥2), product code with external users. A bad merge is real but bounded. | Everything in lite, plus full artefact discipline, memory capture, distinct-actor approval, and a blocking release gate. **The default for an existing consumer that declares no tier (Amendment 8, #1124).** |
