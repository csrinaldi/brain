...
| **L3** | `memory-gate` (memory dumped before closing, ISSUE-SCOPED as of T2.1/#379 — the gate verifies a `session_summary` scoped to the CURRENT change's issue, not just any session summary anywhere; falls back to the pre-T2.1 global existence check when no issue number is detectable) and `decision-gate` (ADR ships for a labeled decision) as required CI jobs | `brain/scripts/governance/run-check.mjs` over `governance/checks/memory-retrieval.mjs` (+ fallback `memory-presence.mjs`) / `adr-presence.mjs` | REQ-L3-1, REQ-L3-2, REQ-L3-3, REQ-L3-4 |
