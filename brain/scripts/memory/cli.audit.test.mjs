// cli.audit.test.mjs — `memory:audit` through the real CLI (#870), against a
...
test('memory:audit prints every row from a fixture store with git history', () => {
...
test('memory:audit --json is the same report as one object', () => {
...
test('memory:audit --since with a non-date exits 1 and says so', () => {
  const run = runCli(fixtureRepo(), '--since', 'nope');
  assert.equal(run.status, 1);
  assert.match(run.stderr, /--since is not a date — nope/);
});

test('memory:audit with a trailing --since (no value) exits 1 — it does not fall back to the default window', () => {
  const run = runCli(fixtureRepo(), '--since');
  assert.equal(run.status, 1);
  assert.match(run.stderr, /--since/);
});

test('memory:audit with --since swallowed by the next flag exits 1', () => {
  const run = runCli(fixtureRepo(), '--since', '--json');
  assert.equal(run.status, 1);
  assert.match(run.stderr, /--since/);
});

test('memory:audit with no records dir exits 1 naming the path — no zeros', () => {
