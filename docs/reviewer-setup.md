...
- **Where it lives locally:** your own shell env, or a gitignored `.env` in
  the repo root (`.env` is already excluded from every commit).
...
Local invocation (H1) reads from the shell env above. A future CI-hosted
runner (H2) will read the same variable from the platform's native secrets
store (GitHub Actions secrets / GitLab CI/CD masked variables) instead —
no code change, only where the value comes from.
