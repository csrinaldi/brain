...
- [x] 3.4 Prove the draft parses and its edit assesses as `blocked` against the pre-#980 doctrine text and `pending` against the post-#980 text, with a throwaway script. Both were measured: `blocked` before this branch merged `origin/main`, and `pending` (`f:1`, `free:1`) against the merged file afterwards, so the ordering constraint is satisfied and promotion remains a separate maintainer act.
