...
- [x] 2.2 Implement D6's answer table in `parseGraphBlock`: one `brain-graph/1` match → parsed object; >1 → `{ok:false}` naming count+lines (REQ-639-3, unchanged); zero + no mention → `null`; zero + a `skipped`/unterminated entry hides a graph-tagged fence → `{ok:false}` naming line+reason.
      > **Checked prematurely, corrected in #723.** The `skipped` half of this line was never implemented: `epic-graph.mjs` did not destructure the field, because PR #720 landed the selector BEFORE PR #722 created it — D0 sequenced them that way on purpose — and nothing came back when the field arrived. Four shapes that HIDE a complete declaration answered `null` until #723. The box was true about the half that was buildable at the time, and nothing said so. **If a requirement is split across a D0-style ordering, the second half has no claimant.**
...
- [x] 2.10 **[HUMAN-ONLY — @crinaldi. No agent may check this box.]** (moved from 7.1) Review the ADR-0032 draft, run `npm run brain:promote -- openspec/changes/issue-709-declaring-selector/brain-drafts/adr-0032-graph-block-declared-by-its-tag.md`, type `PROMOTE`, then run the printed commit command. This commit IS the human signature (ADR-0028). An agent MUST NOT set `status:approved`, MUST NOT add a `**Signed**:` line, and MUST NOT run the commit command on the maintainer's behalf (#124, `actor-check` §9, Tier 3 prohibited per `agent-authorities.md`). **Blocks 2.11 and PR 1 — `local-checks` is red until this lands.**
      > **CHECKED BY @crinaldi, 18/08/2026 — the act and the box now agree.** The promotion had already happened and this box had not been ticked; an agent recorded the evidence in #723 and deliberately left it unchecked, because a checkbox an agent ticks on a human's behalf is exactly the signature this task exists to protect (ADR-0028, #124). Evidence, as verified then: `brain/project/decisions/adr-0032-graph-block-declared-by-its-tag.md` exists with `**Status**: Accepted` dated 2026-08-18; promoted by commit `a4262ec`, authored `Cristian <csrinaldi@gmail.com>`; indexed once in `brain/HOME.md`. The maintainer read that and ticked it. **Two annotations saying "yours to tick" survived the tick and were collapsed into this one** — an archived artefact must not contradict its own checkboxes.
...
- [x] 8.1 **[NEEDS A RENDERED PAGE — human eyes, not a shell]** Place a `​```brain-graph/1` block in one real issue body on GitHub and one on GitLab; observe the rendered page; confirm or refute D13's unverified assumption that an unknown info-string still renders as a fenced code block, losing only highlighting. If false, record it as an amendment to ADR-0032 — the decision does not move, the record does.
      > **CONFIRMED on both providers, 18/08/2026.** Probe: GitHub issue #732 and a matching issue on `gitlab.com/csrinaldi/brain`, each carrying block **A** (` ```brain-graph/1 `) beside a byte-identical control **B** (` ```yaml `), plus a trailing line to reveal a swallowed fence. The control is load-bearing — without it, *renders without highlighting* and *renders wrong* are indistinguishable. Reported identical on both. D13 holds; ADR-0032 is amended to record the closure, not to change the decision.
- [x] 8.2 Re-run `gh pr view 703` and `gh issue view 709 710` to confirm #703 closed and #709/#710's status, before archiving this change (spec.md's stated gap).
      > PR #703 CLOSED, merged=never · #709 CLOSED · #710 OPEN · #723 OPEN, all re-derived 18/08/2026.
- [x] 8.3 Full `npm test` once both PRs have merged; zero regressions repo-wide.
      > `npm test` 4069/4069, 0 fail, with #723's fix and its nine new tests.
- [x] 8.4 `npm run brain:repo:check` before final archive.
      > `brain:repo:check` and `brain:nav` clean.
