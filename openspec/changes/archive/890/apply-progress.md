...
## Batch 5 — maintainer drafts rewritten for brain:promote (2026-09-17)

- `brain-drafts/consolidation-protocol-memory-lane.draft.md` and `brain-drafts/adr-0034-memory-lane-retirement.draft.md` now carry a `brain-amendment/1` block in the exact shape `brain/scripts/lib/amendment-draft.mjs` parses (target, issue; for the ADR: amendment 3, home-summary, body/body-end, three `amend-find`/`amend-replace` acts). Both were dry-run through `runPromote()` with no-op write and stage functions: parse ok, plan ok, guards clean, exit 0, nothing written.
- `brain-drafts/harness-contract-managed-script-keys.draft.md` is removed. Its premise no longer holds: the maintainer dropped `brain:save` from `MANAGED_SCRIPT_KEYS` directly in commit `7ac4e844` (a `.mjs` file is never a valid `brain:promote` target), and `brain/core/methodology/harness-contract.md` carries no `brain:save` reference to amend. Earlier references in this file and in `verify-report.md` to that draft as "the prepared fix" for `managed-script-keys-doctrine.test.mjs` are superseded by that commit; the test is green on the branch.
- Promotion order for the maintainer: consolidation-protocol first, then the ADR-0034 amendment. Both runs regenerate and stage `AGENTS.md` themselves; no separate `brain:env:init` is needed unless a `SOURCE_DOC` is hand-edited afterwards.
