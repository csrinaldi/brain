...
- [x] 1.11 Cold review rev 3 (APPROVE, two nits): the orphan sweep runs on the contended path only — the uncontended fast path pays no `readdir` of tmpdir at every session start; five test call sites carried `_guard` twice (an injection script's leftover), now exactly one each, verified by a per-call scan.
