...
> **DRAFT — agent-authored, awaiting human promotion.** `brain/**` is Tier 2: the agent
> drafts, the human signs. Promote with
> `npm run brain:promote -- openspec/changes/issue-533-epic-map-executor/brain-drafts/adr-0029-two-sources-one-graph.md`,
> which renders this file, shows the destination and the `brain/HOME.md` line, requires a
> typed confirmation, stages the three files and stops. **The commit is the signature.**
>
> **Numbering**: `0029` is the first free number — `brain/project/decisions/` holds up to
> `0028`, and `0018`/`0023`/`0025` are cited elsewhere but absent (the gap ADR-0026's draft
> recorded). Renumber at promotion if you have a claim on it.
