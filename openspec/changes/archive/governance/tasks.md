...
- [x] **S4.1** [CODE-TDD RED] `scripts/governance/checks/diff-size.test.mjs`: test `diffSize(rawNumstat, ignoreList)` — fixture with `.memory/` excluded (→ pass within budget), 401-line fixture (→ fail), binary files → 0. Reuse `parseDiffNumstat` from `diff-size-count.mjs` as the implementation. (REQ-S4-1)
- [x] **S4.2** [CODE-TDD RED] `scripts/governance/checks/issue-link.test.mjs`: (a) body with `Closes #42` → pass; (b) body with no reference → fail; (c) case-insensitive `FIXES #7` → pass. (REQ-S4-1)
- [x] **S4.3** [CODE-TDD RED] `scripts/governance/checks/adr-presence.test.mjs`: (a) changedFiles includes `brain/project/decisions/adr-0042-foo.md` and `brain/HOME.md` → pass; (b) missing ADR file → fail; (c) missing HOME.md → fail. (REQ-S4-1)
- [x] **S4.4** [CODE-TDD RED] `scripts/governance/checks/memory-presence.test.mjs`: (a) changedFiles includes `.memory/chunks/foo.jsonl.gz` → pass; (b) no `.memory/` file → fail. (REQ-S4-1)
- [x] **S4.5** [CODE-TDD GREEN] Create the four check modules in `scripts/governance/checks/`: `diff-size.mjs` (re-exports `parseDiffNumstat`), `issue-link.mjs`, `adr-presence.mjs`, `memory-presence.mjs`. Each returns `{ pass: boolean, reason?: string }`. (REQ-S4-1)
- [x] **S4.6** [FILE] Create `scripts/hooks/commit-msg`: validate conventional commit format + ticket ref (`#N` required for non-merge commits); no-node guard (exit 0 if node unavailable). (REQ-S4-2) — DEVIATION: S4.8 hard-blocks only locally-unambiguous items (see note below).
- [x] **S4.7** [FILE] Create `scripts/hooks/pre-commit`: run repo:check via node directly; block direct commit to `main`/`master` by reading current branch via `git rev-parse --abbrev-ref HEAD`; no-node guard. (REQ-S4-3)
- [x] **S4.8** [FILE] Extend `scripts/hooks/pre-push`: **RELIABILITY-FIRST DEVIATION** from literal spec. Hard-blocks: (a) uncommitted .memory/ [existing], (b) repo:check [new]. Diff-size is a WARNING only. issue-link, adr-presence, memory-presence NOT wired in pre-push (false-positive risk → --no-verify). brain:audit is the hard enforcement layer. (REQ-S4-4)
- [x] **S4.9** [CODE-TDD + FILE] `scripts/brain-audit.mjs` + test: (a) iterate merge commits in audit range via `git log --merges`; (b) run four checks per merge; (c) emit `[PASS|FAIL] <sha> <short-msg> — <failed invariants>`; (d) exit non-zero on any FAIL. 3-test fixture suite (PASS merge, FAIL merge, no-merge range). `"brain:audit"` added to `package.json`. (REQ-S4-5, REQ-S4-6)
- [x] **S4.10** [OPERATOR] `npm test` → 318/318 pass (all check unit tests + audit fixture tests + all prior tests). repo:check green, brain:nav green.
...
> **S4.8 deviation note**: Pre-push wires ONLY (a) memory materialization, (b) repo:check, and (c) diff-size WARNING. issue-link/adr-presence/memory-presence hard-blocks are NOT in pre-push because at push time there is no PR body, the PR may be split afterwards, and ADR judgment is human — false-positives lead developers to --no-verify, defeating governance. brain:audit enforces all 4 invariants hard on the merged history (the canonical enforcement point). S4.6 (commit-msg) and S4.7 (pre-commit) hard-block because those checks ARE locally unambiguous (format, branch name, reference validity).

> **S4 size note**: Total S4 diff is ~497 lines (S4a: 182 + S4b: 315). Split into two commits on gov/s4-floor per budget rules. S4a (checks library) committed at 6066aa8; S4b (hooks + brain:audit) committed separately.
...
- [x] **S5.1** [CODE-TDD RED+GREEN] `scripts/brain-start.mjs` + test: (a) call `issueView()` via VCS adapter; (b) assert `status:approved` label → create branch via `branchCreate`; (c) exit non-zero with clear message if not approved or not found. Add `"brain:start": "node scripts/brain-start.mjs"` to `package.json`. (REQ-S5-1)
- [x] **S5.2** [CODE-TDD RED+GREEN] `scripts/brain-check.mjs` + test: spawn `diffSize`, `issueLink`, `adrPresence`, `memoryPresence` against current branch diff vs. base; spawn `npm test`; spawn `npm run repo:check`; aggregate; exit non-zero if any fails. Add `"brain:check": "node scripts/brain-check.mjs"` to `package.json`. (REQ-S5-2)
- [x] **S5.3** [CODE-TDD RED+GREEN] `scripts/brain-save.mjs` + test: (a) call `memory:share`; (b) detect new `.memory/` uncommitted changes via `git status --porcelain`; (c) if none → exit non-zero with prompt to capture session summary; (d) commit `.memory/` with message `chore(memory): sync .memory [brain:save]`. Add `"brain:save": "node scripts/brain-save.mjs"` to `package.json`. (REQ-S5-3)
- [x] **S5.4** [CODE-TDD RED+GREEN] `scripts/brain-ship.mjs` + test: (a) call `brain:check` subprocess; (b) exit non-zero if any check fails; (c) call `mrCreate` via VCS adapter with template + `Closes #<issue>` body + labels; (d) print PR URL on success. Add `"brain:ship": "node scripts/brain-ship.mjs"` to `package.json`. (REQ-S5-4)
- [x] **S5.5** [CODE-TDD RED+GREEN] `scripts/brain-next.mjs` + test: derive state from (git branch, open PRs via VCS adapter stub, `.memory/` status, brain.config.json); emit correct next command for each state: no-branch → `brain:start`; checks-failing → `brain:check`; no-memory → `brain:save`; checks+memory done → `brain:ship`; open-PR → status message. Add `"brain:next": "node scripts/brain-next.mjs"` to `package.json`. (REQ-S5-5)
- [x] **S5.6** [FILE + CODE-TDD] Add `--no-verify` and `git commit -n` to the prohibited-refs list in `brain/project/check-refs-rules.mjs` (the config file used by `repo:check`). Added unit test `check-refs.test.mjs` asserting `repo:check` exits non-zero on a fixture file containing `--no-verify`. (REQ-S5-6)
- [x] **S5.7** [FILE] Created `.claude/settings.json` with a PreToolUse hook rule blocking Bash commands matching `--no-verify|git commit -n`. Path added to `brain/core/managed-paths.mjs` so it travels with brain. (REQ-S5-6)
- [ ] **S5.8** [OPERATOR] `npm test` → green (all S5 tests pass). 353/353 pass — CODE TASKS DONE.
