...
Note `skip:memory-gate` is documented in `AGENTS.md` but read by **no code path** today
(`brain-metrics.mjs` explicitly reports it raw and never subtracts it). Tiering it is
specifying a label that must first be made real; until then the `regulated` column is
the only one that is enforceable-by-construction (it refuses a label nothing honors).
...
**Alternative, if the human prefers epic-literal.** Set `standard` to
`proposal + spec + design` (drop only `tasks.md` from Rule A, which is harmless
because Rule C independently requires `tasks.md` to have ≥1 checked item whenever
implementation code is present). REQ-L4-2 survives, the epic's spirit is closer, and
`regulated` still needs the verification artefact for a real delta. This is the
compromise position; the primary recommendation is preferred because it changes
nothing about the default.

**Do not adopt `proposal + spec` for `standard`.** It is the one option that
weakens the default tier while claiming to strengthen governance.
...
**Concrete acceptance against #329's own criteria:**

- *"An ADR records the chosen resolution and supersedes/amends whatever L5 doctrine it
  contradicts"* → `brain-drafts/adr-0026-governance-doctrine-tiers.md` amends
  ADR-0015's REQ-L5-1 and REQ-L6-1 (evidence tiering) and REQ-L4-2 (artefact set).
- *"`actor-check` produces a defensible verdict for a solo maintainer — green, or a
  documented warn — rather than a permanent fail"* → **green**, on distinct-act
  evidence, with the tier and evidence form named in the verdict. Strictly better than
  the warn the issue would have settled for.
- *"reflected in `docs/KNOWN-LIMITATIONS.md` if any limitation remains"* → the
  remaining limitation is not the gate; it is that `standard`/`regulated` are
  unexercised at n=0 external adopters (§8).
...
On the budget: brain's diff discipline is a habit worth keeping at 400. REQ-TIER-1's
monotonicity forbids a per-gate override, so the honest options are (a) accept 1000 at
`lite` and keep 400 as a convention enforced by the chained-PR workflow, or (b) set
`lite`'s budget to 400 and give `standard` something else. **Recommend (a)** — the
budget is the tier's most visible proportionality signal and lowering `lite`'s
ceiling to `standard`'s would collapse a column.
...
2. **Retrieval itself is never tiered.** Retrieval is a *read* capability with no
   ...
   set**:
   | Tier | What `memory-gate` asserts post-T2.1 |
   |---|---|
   | `lite` | Retrieval works; the gate reports coverage (detection) |
   | `standard` | The PR's linked issue has ≥1 memory record referencing it |
   | `regulated` | Plus: the record is a `session_summary` and predates the merge commit |
3. **T2.1 MUST land `issue`-field population, or the standard-tier assertion is a
   ...
   non-zero would red every PR in every repo. This is textbook M10 "green in test,
   inert (or lethal) in production": the fixtures would carry an `issue` field that
   production records do not.
...
**T2.1 deliverable shape**: `resolveGatePolicy('memory-gate', tier)` and
`tierParams(tier).memoryAssertion` consumed by the memory check; retrieval itself
built with no tier awareness at all.

## §7 — M3 reviewer-gate impact

| Concern | Tier impact |
|---|---|
| **#317 (`prReviews` strips `body`)** | **Tier-independent, and it lands first.** `priorVerdicts` is always empty at *every* tier, so anti-loop, the rev≥3 bound, and board reconciliation are inert everywhere. Tiers do not help — it is a seam bug. Nothing in M3 should be sequenced behind Q5 that could instead be sequenced behind #317. |
| **#284 (reviewer v2: refuter + causal admission)** | **Reframed by this decision.** The `/2` schema and `refuter.mjs` are dead code today (the CLI never passes `protocol`, defaulting to `/1`). Under tiers they become **`regulated`'s enabling work** — the panel-≥2 consensus mode. #284 stops being a nice-to-have and becomes the reason the top tier exists. |
| **Reviewer merge authority** | **Never tiered.** `event: COMMENT` (ADR-0020) is a REQ-TIER-2 member. No tier may make the reviewer an approver — that would collide with L5 and #124 (approval is a human signature). |
| **Reviewer as a governance job** | **Never `required`, at any tier.** A required job that is COMMENT-only by construction is an unblockable check. At `standard`/`regulated` the reviewer verdict is `detection`. |
| **How does `regulated` enforce panel review, then?** | By enforcing the **artefact**, not the tool: the recorded verification artefact (§2.C) must exist. This is ADR-0014's enforce-outputs / guide-judgment boundary and REQ-NEUTRALITY-1 (inspect evidence, never the producing tool) applied unchanged. A regulated shop using a different reviewer satisfies the gate; brain never asserts its own tool is mandatory. |
| **Inline per-line comments · `/1`-vs-`/2` schema contradiction · routing `brain-governance-status` through the vcs port** | Tier-independent M3 work. Unaffected by this decision in either direction. |
...
and the branch-protection list cannot disagree by construction. Two mechanisms are
needed, not one: `checkContexts(tier)` alone leaves a `lite` repo at rung 4 showing a
red `actor-check` (there is no protection to omit it from), which is #329's original
complaint. Per-job exit policy is what produces an honest green/warn at *every* rung.

**Drift-guard test (REQ-TIER-8)**: every `GOVERNANCE_JOBS` name has a full matrix row;
every matrix row names a job the workflow defines. Fail-closed — adding a job forces a
tier decision instead of silently inheriting one.
...
- [ ] **`decision-gate`'s shipped check does not match its documentation.**
      ...
      second step. §2.A's `standard` evidence ("plus the `decision`-label step hard")
      describes the *documented* behaviour, which is not the *shipped* behaviour. This
      pre-existing divergence must be resolved (fix the code or fix the doc) before
      that evidence row means anything.
...
- [ ] **`override:*` must be tier-scoped in the same slice as the evidence forms.**
      If evidence tiering lands while `override:*` stays unconditional, `lite` repos
      will keep passing by bypass and the improvement is cosmetic.
