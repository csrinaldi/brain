...
> **DRAFT — agent-authored, awaiting human promotion.** Per ADR-0013 (Tier 2), an agent
> never writes to `brain/`. Promote to `brain/project/decisions/adr-0026-governance-doctrine-tiers.md`
> together with the `brain/HOME.md` index entry in the SAME PR (`decision-gate` step 1).
>
> **Numbering caveat**: `0018`, `0023`, and `0025` are each cited somewhere in the repo
> but absent from `brain/project/decisions/`. `0026` is the first number free of any
> claim. Renumber at promotion if the human has a claim on it.

**Status**: Proposed
**Date**: 30/07/2026 — draft for Cristian Rinaldi
...
- **Negative (pre-existing, now load-bearing)**: `decision-gate`'s shipped check
  (unconditional ADR ⇔ `HOME.md` co-occurrence) does not match its documentation
