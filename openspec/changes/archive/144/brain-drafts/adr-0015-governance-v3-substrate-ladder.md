> DRAFT for human review — pending promotion into `brain/project/decisions/` per
> Tier 2 (`agent-authorities.md`). This file lives under `openspec/changes/**` because
> agents do not write directly to `brain/project/**`; a human copies/renames this into
> `brain/project/decisions/adr-0015-governance-v3-substrate-ladder.md` (and updates
> `brain/HOME.md`) after review. This is precisely the constraint the change itself
> enforces (L6, §6 below) — the ADR about "no agent writes to `brain/`" is, itself, not
> written to `brain/` by the agent that drafted it.

# ADR-0015 — Governance v3: Six-Level Fail-Closed Gate Ladder Over Observable Evidence

**Status**: Proposed (draft — pending human review and promotion)
**Date**: 2026-07-01
