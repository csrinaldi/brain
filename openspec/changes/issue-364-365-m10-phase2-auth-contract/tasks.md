...
- [ ] **1.1** Record `github-authCheck-happy.json` from a real, non-mutating `gh auth status`
...
- [ ] **1.2** Hand-author `github-authCheck-failure.json` (`_provenance.derived` — forcing a real
...
- [ ] **1.3** Hand-author all four `{github,gitlab}-authLogin-{happy,failure}.json`
...
- [ ] **1.4 GATE** — Confirm all eight fixtures pass `assertProvenance` (exactly one of
...
- [ ] **2.1 RED** — Add `rawStatusCallArgs(fixture)` glue: `setSpawn(() => ({ status:
...
- [ ] **2.2 RED** — Add `authCheck` happy/failure tests inside the parity loop: `assert.equal(result,
  ...
  `mrList`/`issueList`.
- [ ] **2.3 RED** — Add `authLogin` happy/failure tests, same shape, called with `{ host:
  'github.com', token: 'test-token', ...rawStatusCallArgs(fixture) }`.
- [ ] **2.4 RED** — Add the `authCheck` host-omission divergence test (no fixture): capture
...
- [ ] **2.5 RED** — Add the `authLogin` host-default divergence test (no fixture): same capture
...
- [ ] **2.6 RED** — Add the `authLogin` token-via-stdin parity test (no fixture, both providers in
...
- [ ] **3.1 GREEN** — Register `authCheck: rawStatusCallArgs` and `authLogin: rawStatusCallArgs`
...
- [ ] **3.2 GATE** — The Phase 2 fixture-driven tests now pass (8 assertions: happy/failure × 2
...
- [ ] **4.1** Amend `vcs-contract.md` rows 24–25 (`authCheck`/`authLogin`): document the
...
- [ ] **4.2 GATE** — `npm test` on the full suite: 11 new tests green (8 fixture-driven + 3
