...
| **2. Evidence shape** | Structured `RunLedgerEvidence` (below) + a 13-row total decision table | Verdict-in-probe is excluded by #337's D1 split (`brain-governance-status.mjs:91-99`). A `verifiable:false` "declared-but-unverified" armed on read failure is excluded by the settled product decision (semi-green reads as health). |
...
| E1 | `workflowPresent !== true` — includes the field being explicitly `false` AND the field being omitted entirely (`read:'skipped'`, or malformed/partial probe output) | true | false | true | `postmerge-ci-absent` |
| E2 | `read:'unsupported'` (non-GitHub provider) | **false** | false | true | `postmerge-run-ledger-unsupported` |
| E3 | `read:'failed'` (auth / no token / network / rate limit / bad JSON — including a parseable-but-wrong-shaped 200 body where `workflow_runs` is missing or not an array) | **false** | false | true | `postmerge-run-ledger-uncomputable` |
| E-fallback | `read` is present but none of `'skipped'/'unsupported'/'failed'/'ok'` — an unrecognized read state from a malformed or future-shaped probe | **false** | false | true | `postmerge-run-ledger-uncomputable` |
| E4 | `read:'ok'`, `lastRun: null` (zero terminal runs) | true | false | true | `postmerge-unproven` |
| E5 | `lastRun` malformed — no `conclusion`, unparseable `completedAt`, or `observedAt` not a **finite** number (`NaN`, `Infinity`, `-Infinity`, `null`, or missing) | **false** | false | true | `postmerge-run-ledger-uncomputable` |
| E6 | `conclusion !== 'success'` | true | false | true | `postmerge-failing` |
| E7 | success, clock skew: `observedAt − completedAt < -POSTMERGE_CLOCK_SKEW_TOLERANCE_MS` (a future-dated `completedAt`, or a local clock running behind by more than the tolerance) | **false** | false | true | `postmerge-run-ledger-uncomputable` |
| E8 | success, `observedAt − completedAt > POSTMERGE_STALE_MS` (and not caught by E7) | true | false | true | `postmerge-stale` |
| E9 | success, age within `[-POSTMERGE_CLOCK_SKEW_TOLERANCE_MS, POSTMERGE_STALE_MS]` | true | **true** | true | `postmerge-run-ledger` |

Only L1 and E9 arm. L1 is unreachable in production — the real probe always returns an object (locked by a fixture test). Every uncomputable row keeps a non-empty `reason` **and** `remedy`, required by `substrate.test.mjs:28-36` (top-level `reason`/`remedy` come from `rungs[3]` at rung 4, `substrate.mjs:529`).

**Non-finite `observedAt` (E5, hardening).** `typeof observedAt !== 'number'` is NOT sufficient to guard staleness, because `typeof NaN === 'number'` — a `NaN` (or `±Infinity`) `observedAt` sails past a naive `typeof` check, produces a `NaN` `age`, and `age > POSTMERGE_STALE_MS` is `false` for `NaN`, so control falls all the way through to the arming row. `evalRung3` uses `Number.isFinite(observedAt)` instead, so any non-finite value is caught at E5 as uncomputable.

**Clock skew (E7, hardening).** `observedAt` is the LOCAL wall clock; `completedAt` is GitHub's. A future-dated `completedAt`, or a local clock running behind (VM resumed from suspend, container without NTP, WSL drift), yields a negative or understated `age` that would otherwise satisfy `age ≤ POSTMERGE_STALE_MS` and arm on a genuinely stale (or unprovable) run. `brain:governance-status` runs on a local developer machine, making this the most likely live path to a false arm — so a negative age beyond a small, named tolerance is treated as uncomputable, never as "fresher than fresh".

```js
export const POSTMERGE_CLOCK_SKEW_TOLERANCE_MS = 5 * 60 * 1000; // 5 minutes
```

Chosen small (minutes, not hours) deliberately: it only needs to absorb ordinary clock imprecision and network round-trip, not mask a real clock fault. A skew large enough to need a bigger tolerance is itself a signal worth surfacing as uncomputable rather than silently absorbing.

**Unrecognized `read` state (E-fallback, previously-undocumented defensive branch).** If a probe returns a `read` value outside the 4-state contract (`skipped`/`unsupported`/`failed`/`ok`) — a malformed probe, a future schema change not yet handled here — `evalRung3` fails closed to uncomputable rather than guessing which of the four states was intended. This keeps the decision table total: no input class silently falls through unmapped.
...
1. extract EVERY cron entry: `[...text.matchAll(/-\s*cron:\s*'([^']+)'/g)]` → `['0 6 * * *']` (`governance-postmerge.yml:26`) — `matchAll`, not a single non-global `.match()`, because a single match only inspects the FIRST entry: adding a second `- cron:` line (e.g. an hourly cron alongside the daily one) would change the effective cadence while entry #1 stayed daily-shaped and the guard silently kept passing;
2. assert exactly one cron entry is declared — multiple schedule entries change the effective cadence and require re-deriving `POSTMERGE_STALE_MS`;
3. assert that one entry is still daily-shaped: `/^\d+\s+\d+\s+\*\s+\*\s+\*$/`;
4. assert `POSTMERGE_STALE_MS === 2 * 24 * 60 * 60 * 1000`.

Any cadence edit — a changed cron string, OR an added/removed cron entry — fails one of these assertions with a message naming the constant, so the two cannot drift silently. No cron parser is introduced (zero-dependency doctrine, `substrate.mjs:111-114`).
...
Freshness and skew (E7/E8/E9) are asserted only at the pure layer with injected `observedAt`; real-probe fixtures own evidence extraction, never time. This avoids templated timestamps in static JSON fixtures.
