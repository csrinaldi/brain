...
> **status:** proposed — pending human promotion | **date:** 2026-08-17 | **owner:** @crinaldi
> **amends:** nothing | **supersedes:** REQ-639-1's selector sentence; corrects ADR-adjacent doctrine in `openspec/changes/issue-495-declared-budget-claim/design.md` D1

> **Promote with the verb, do not move the file.**
>
> ```
> npm run brain:promote -- openspec/changes/issue-709-declaring-selector/brain-drafts/adr-0032-graph-block-declared-by-its-tag.md
> ```
>
...
> renders this draft in full, shows every act with its before and after, requires
> the typed word `PROMOTE`, then **stages and stops**. Running the commit command
> it prints is the human signature. No agent applies `status:approved` and no agent
> signs this file; a claimed identity establishes nothing, which is the rule
> `actor-check` enforces and ADR-0031 explains.
>
> Copying the file by hand instead would skip §1d's cascade — including the
> `brain/HOME.md` index entry that `decision-gate` requires for an **added** ADR
> and the `AGENTS.md` regeneration. A hand-rolled promoter lost exactly that step
> once before, which is why the verb exists.
>
