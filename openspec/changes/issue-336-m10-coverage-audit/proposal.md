...
- [ ] Every verb in `vcs-contract.md`'s Required Verbs table appears in the audit — count matches the drift guard.
- [ ] Each row states contract-parity coverage AND fixture provenance as separate values.
- [ ] Top 6 Phase-2 candidates are ranked with a stated rule, `prReviews` first.
- [ ] Zero-coverage verbs (`authCheck`, `authLogin`) are flagged with their narrow blast radius stated.
- [ ] Fixture debt section names the unvalidated GitLab side explicitly.
- [ ] The diff contains no changes outside the change folder.
