...
> PR #966) before Amendment 1 promoted, and the same act annotated every superseded line in the
...
The anchor is the two-line sentence at `adr-0034-memory-travels-on-its-own-lane.md:267-268`, the
only occurrence of `npm run` in this ADR. Verified against this ADR on this branch with
...
mapping. **[Amended by Amendment 2 (#973) — rewritten: the previous sentence said the body was
not rewritten, although the same act had annotated it in place.]** No line of this ADR runs a
script with a literal `npm run`, so the decision reads the same.
```
