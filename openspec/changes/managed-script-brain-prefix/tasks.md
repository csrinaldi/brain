...
## Phase 7: Doctrine drafts — annotated, not on the Tier-1 critical path

- [x] 7.1 **Resolved 2026-09-14 — maintainer amended R6 (option A).** Ruling gap (`design.md` "RULING GAP"): each ADR line naming a bare `memory:*` script is annotated in place via one `§1c` `amend-find`/`amend-replace` pair per line (verified issue #961 comment, 2026-09-14; engram `sdd/managed-script-brain-prefix/ruling-r6-amendment`). The five drafts (`brain-drafts/adr-000{2,11,14,17,34}-amendment-*.draft.md`) each carry their full anchor set (7/1/1/11/9 pairs = 29 pairs over 31 annotated occurrences; two adjacent-name lines in `adr-0017` share one combined anchor each) and the appended signed amendment with its rename table. `brain-drafts/README.md` and this file record the resolution.
- [x] 7.2 Prove each draft promotable WITHOUT promoting: `parseAmendmentDraft` ok on all 5; every edit's `assessEdit` reports `pending`/`free: 1` against the real target ADR on this branch; `applyEdits` succeeds, introduces no `brain:brain:`, and leaves `adr-0002…:86` and `adr-0034…:136,143` (already `brain:memory:ship`) byte-identical. Never ran `npm run brain:promote` — proof only, via the real `amendment-draft.mjs` functions from a scratch script. All 5 PASS (see `apply-progress.md`).
- [ ] 7.3 Doctrine promotion itself (`brain:promote` on the 7 core drafts + the 5 ADR drafts + the 2 R8 hand edits) is a separate maintainer PR, out of scope for `sdd-apply`, after the Tier-1 PR merges (R1, R10). Owner: maintainer.
