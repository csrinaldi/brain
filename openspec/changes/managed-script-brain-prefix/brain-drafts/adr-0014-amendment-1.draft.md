...
The anchor names the only occurrence of `memory:share` in the target (line 65), verified with `assessEdit`
(`free = 1`).
...
```amend-replace
the PR (⇒ `memory:share` (renamed `brain:memory:share`; see Amendment 1) ran).
```
