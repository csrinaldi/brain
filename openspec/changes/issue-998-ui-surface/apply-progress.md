...
### Cold review of PR 5 (#1009): REVISE → fixed

Branch `feat/issue-998-pr5-reviews`, worktree `brain-issue-998-5`, head `0f5d4480` (merge of `origin/feat/issue-998-pr4-sdd`) before this round.

| sha | finding | unit | RED → GREEN | mutation |
|---|---|---|---|---|
| 32616c33 | 1 (blocker) | `review-timeline.mjs`'s `shapeRound` copied only `rev`/`verdict`/`headSha7`/`author`/`findings`/`bySeverity` from `reviewRows`' output, dropping `malformed` and `findingCount` — a malformed round reached the Reviews mode as `{findings: [], bySeverity: {}}`, identical to a clean verdict with zero findings, and `app.js`'s `renderReviewRound` said "no findings" for both. `shapeRound` now also carries `malformed` (array, possibly empty) and `findingCount` forward. `app.js`'s `renderReviewRound` checks `round.malformed` before the empty-findings case and says "findings block unreadable: `<keys>`" (marked ⚠) instead. `drawer-model.mjs`'s `reviewEntries` names the same unreadable keys rather than folding them into the generic "unknown finding count" phrase | RED → GREEN across `review-timeline.test.mjs` (1 new), `drawer-model.test.mjs` (1 new), `views-owned.test.mjs` (1 new text-scan asserting the malformed check precedes the empty-findings check in `app.js`'s source) | `shapeRound`: dropped the `malformed` copy → the new round-shape test red; reverted |
| 795254c4 | 2 (correction) | `waitingOn` returned `thread.latest.headSha7` for a REVISE-latest thread and `null` for "not waiting" — `headSha7` is `null` when `head_sha` is not a string, so a REVISE thread with an unparseable head was silently excluded from the queue, encoding "waiting" and "sha unknown" as the same value. `waitingOn` now returns `{waiting, head}`; the queue filters on `waiting`, and a queue entry whose head could not be read says `wait: 'head not readable'`, distinct from the noRound case's `'no round posted'` | RED → GREEN: a new test posts a REVISE verdict with `head_sha: null` and asserts the thread still enters the queue with that literal | the queue filter changed from `w.waiting` to `w.head !== null` → red (the new test, plus the pre-existing noRound queue test, since noRound also carries `head: null`); reverted |
| 8161e1fa | 3 (editorial) | `parseFindingLine` accepted `n >= 0`, but the emitter's own invariant (`hasUsableAnchor`, `verdict.mjs`) never posts a finding with `line: 0` — only `line >= 1` — and `provenance.mjs`'s stamp uses a falsy check (`source.line ? ... : ...`), so a `line: 0` would silently render as `[repo: path]` with the line dropped. `parseFindingLine` now accepts only integers `>= 1`; the comment states the emitter's invariant it matches | RED → GREEN: a new test posts a finding with `line: 0` and asserts it parses to `null` | `>= 1` reverted to `>= 0` → the new test red; reverted |

`openspec/changes/issue-998-ui-surface/spec.md`'s R998-5 gained two scenarios this round ("a REVISE thread with an unreadable head still joins the queue", "a malformed round renders as unreadable on screen, never as zero findings") plus one for finding 3 ("a finding's line of 0 is not a usable anchor") — the existing text described `reviewRows`' own `malformed`/`findingCount` fields and the queue's normal REVISE case, but not what the shaped round or the on-screen render do with them, and not the line-0 edge case.

Verification: the full UI glob (`brain/scripts/ui/**/*.test.mjs` `brain/scripts/ui/*.test.mjs` `brain/scripts/status/snapshot.test.mjs` `brain/scripts/status/snapshot-cli.test.mjs`) 310/310 GREEN; `npm run brain:repo:check` green before every commit; full suite `GIT_CONFIG_GLOBAL=/dev/null npm test` once: 5671 tests, 5670 pass, 1 fail — same unrelated pre-existing failure as PR 4's round, `session-end-ship.test.mjs`'s "real entrypoint … writes no log file" (this machine's real tmpdir already carries private dirs from unrelated prior sessions; #1011). Counted diff `origin/feat/issue-998-pr4-sdd...HEAD` (tests, `openspec/`, `.memory/` excluded): 322/1000 (was 270 before this round). Working tree clean after every commit; no push, no PR opened, no `--force`.
