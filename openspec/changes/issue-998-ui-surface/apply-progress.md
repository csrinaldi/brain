...
Full suite `GIT_CONFIG_GLOBAL=/dev/null npm test`: 5654/5655 after the correction (PR 4 left it at 5639/0; +15 new tests net over the batch, +2 from the correction's own tests). The one failure remains `session-end-ship.test.mjs`'s real-entrypoint assertion — unrelated (see below, unchanged). `npm run brain:repo:check` green before every commit. Counted diff `072c0861...HEAD` (tests, `openspec/changes/**`, `.memory/**` excluded): recomputed after the correction, see the return summary for the exact figure.
