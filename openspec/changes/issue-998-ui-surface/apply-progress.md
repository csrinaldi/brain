...
### Fresh-context review before push: APPROVE, two minors fixed

| sha | finding | unit | RED → GREEN | mutation |
|---|---|---|---|---|
| 970ab9f | minor 1 | `change-route.mjs:181-188`'s `buildSddTab`: the row-not-found branch (`issue N has no row in the changes section`) was dead — `dir` already resolves via the same `snapshot.changes.find(c => c.issue === issue)` predicate `findChangeDir` uses, and `readChanges` never yields a row without a `dir`, so `!dir` (`noChangeDirTab`) always fired first; `change-route.test.mjs:380-391`'s `assert.match(reason, new RegExp(issue))` passed only because both reason strings happened to contain the issue number. Removed the dead branch (one reason, said once) and tightened the test to `assert.deepEqual` against the exact `noChangeDirTab` shape | the tightened exact-match test already passed against the OLD code (proving the coincidence); GREEN 18/18 unchanged after removing the dead branch | reworded `noChangeDirTab`'s reason string → both the tightened sdd test AND the pre-existing spec/tasks test went red (16/18); reverted |
| ffe255a4 | minor 2 | `poller.mjs:92-96`'s comment claimed a manual `once()` "does not itself re-arm the interval" — measured false: `once()` at t=5000 with a 60000ms interval leaves `nextAttemptAt` at 65000, because `runTick()`'s own `.finally()` calls `scheduleNext()` on every completed tick, manual or scheduled alike. Reworded the comment to the measured behaviour; added the missing test (`once()` at t=5000 → `nextAttemptAt === 65000`) | the new test already passed (behaviour was already correct, only the comment was wrong) → GREEN 16/16 | reset `nextAttemptAt` to `null` right after `once()`'s `runTick()` call → red, exactly its own test (15/16); this specific mutation is cosmetic — the real timer `scheduleNext()` already armed inside `runTick()`'s `.finally()` stays intact, so it does not itself stall future polling; a mutation that skipped calling `scheduleNext()` entirely from `once()`'s path (bypassing `runTick()`) would stall polling, since no future tick would ever be scheduled — confirmed this alternate mutation does not disturb any OTHER poller/server test (checked `server.test.mjs`'s 46/46 stayed green throughout); reverted |

Verification: the UI glob (`brain/scripts/ui/**/*.test.mjs` `brain/scripts/ui/*.test.mjs`) 300/300 GREEN; `npm run brain:repo:check` green before both commits; tree clean after each.

### Carried from #1008's verdict

PR 6 descends from PR 4's head, and PR 4's cold verdict (#1008, APPROVE with a correction and an editorial) carries forward onto this branch since PR 4 merges as approved.

| sha | finding | unit | RED → GREEN | mutation |
|---|---|---|---|---|
| c2b14c25 | correction | `status/derive.mjs`'s `deriveTasks()` (lines 84-90) renders "zero open items" as the literal em dash `'—'` — its own display sentinel, printed verbatim by other consumers (left unchanged, per instruction). `sdd-model.mjs`'s `buildTasksSummary` forwarded it unchanged, so `static/app.js:382`'s `renderSddRow` rendered `tasks: 2 checked, 0 open — next: —` for every done or task-less change. Normalised the sentinel to `null` in `buildTasksSummary` (`NO_NEXT_TASK_SENTINEL`, with a comment naming where it comes from); the row's existing `t.next ? … : ''` truthy guard already omits the fragment once `next` is `null` — no `app.js` change needed. Added a pinning scan test in `sdd-view.test.mjs` confirming that guard exists (it already did; kept) | RED (a `next: '—'` fixture asserted `null`, 1/17 failing) → GREEN 17/17 across `sdd-model.test.mjs`; `sdd-view.test.mjs`'s new guard-scan test GREEN 4/4 (no behavior change there) | forwarding the value verbatim (dropping the sentinel check) → red, exactly the new test (16/17); reverted |
| fd9392ed | editorial | `status/snapshot.mjs:148`'s comment above the (now-deleted) `specPresentAt` claimed `hasSpec`'s own `changeDir(changeId)` path-building "does not go through" the `archive/<issue>` location — measured false: `changeDir('archive/<n>')` and `archivePath(<n>)` template the identical string from the same `CHANGES_ROOT` constant (`${CHANGES_ROOT}/archive/<n>` either way). Deleted the duplicate `specPresentAt` (preferred over correcting the comment, since the existing tests stayed green) and delegated to `sdd-layout.mjs`'s own `hasSpec`, threaded through `missingId` — already the identifier `missingRequiredArtifacts` resolves a path from for both active (`id`) and archived (`archive/<name>`) rows, so no new identifier was needed | added one new coverage case first (an archived row with the nested `specs/*/spec.md` convention resolves `spec:true`) — passed against the OLD `specPresentAt` code too (establishing the baseline the refactor had to preserve); GREEN 24/24 across `snapshot.test.mjs`/`snapshot-cli.test.mjs` unchanged after the refactor | passed `id` instead of `missingId` into `readArtefactPresence` (the exact archived-row bug the deleted comment's false claim would have masked) → both the pre-existing archived-row test AND the new nested-spec test went red (19/21); reverted |

Verification: UI + status glob (`brain/scripts/ui/**/*.test.mjs` `brain/scripts/ui/*.test.mjs` `brain/scripts/status/**/*.test.mjs`) 466/466 GREEN; `sdd-layout.test.mjs`'s real-tree drift guard 78/78 GREEN (the `hasSpec` import change did not introduce a new rival array); `npm run brain:repo:check` green before both commits; tree clean after each.

Full suite `GIT_CONFIG_GLOBAL=/dev/null npm test` after all four fixes: 5684/5685 (the one persistent, unrelated `session-end-ship.test.mjs` failure, unchanged). Counted diff `0f5d4480...HEAD` (tests, `openspec/changes/**`, `.memory/**` excluded, across the whole PR): 217/400.
