...
- [x] T3a. `ui/lib/drawer-model.test.mjs` extended: a review entry's `children` carry one row per finding (`severity — id`, the excerpt and `cites` in `detail`), inheriting the round's own `source` (no per-finding anchor exists in this provider, D14); the detail's finding count reads `findingCount`, not `findings.length`. RED.
