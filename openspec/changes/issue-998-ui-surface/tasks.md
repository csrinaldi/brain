...
- [x] T6. `npm run memory:save -- "<title>" "<content>" --issue 998 --type decision`; stage only the record and `.memory/index.jsonl`.
...
- [x] T5. `npm run memory:save -- "<title>" "<content>" --issue 998 --type decision`; stage only the record and `.memory/index.jsonl`.

## PR 5 — findings per verdict, the reviews timeline, the queue (R998-5)

```brain-slice-scope/5
{"slice": 5, "claims": ["R998-5"], "files": ["brain/scripts/status/snapshot.mjs", "brain/scripts/ui/lib/review-timeline.mjs", "brain/scripts/ui/lib/drawer-model.mjs", "brain/scripts/ui/lib/view-model.mjs", "brain/scripts/ui/static/app.js", "brain/scripts/ui/static/app.css"], "terminal_pr": "the tracker feature/issue-998-ui-surface -> main"}
```

- [x] T1a. `status/snapshot.test.mjs` extended: `reviewRows` carries each verdict's `findings` as the SHAPED ARRAY (`{id, severity, evidenceExcerpt, cites}`, excerpt truncated to 240 chars), not the count — plus a `findingCount`; a verdict with two findings survives the round trip through `buildSnapshot`; a verdict whose `findings:` block is malformed keeps `malformed: ['findings']` and `findings: []`. RED (asserting an array where the count used to be).
- [x] T1b. `status/snapshot.mjs`: `reviewRows` maps `parseVerdict`'s `v.findings` entries to `{id, severity, evidenceExcerpt, cites}` (~~no fabricated `file`/`line` — the protocol schema, `reviewer-protocol.md` §6.1/6.2, declares no such fields; only `evidence`/`cites` are real~~ **CORRECTED, same session, coordinator-flagged**: `file`/`line` ARE real emitted fields — `verdict.mjs`'s `renderVerdict` posts them per finding when `hasUsableAnchor` holds, issue #405/REQ-405-2; measured on PR #1006's posted verdict — so the shaped finding is `{id, severity, evidenceExcerpt, cites, file, line}`, both nullable) and adds `findingCount`. GREEN across `snapshot.test.mjs`/`snapshot-cli.test.mjs`. Mutation: the array replaced by its `.length` → the two-finding parity assertion red; reverted.
- [x] T2a. `ui/lib/review-timeline.test.mjs`: `buildReviewTimeline(reviewsSection, prsSection, {issue?})` — a thread with two rounds (REVISE then APPROVE), each round's findings grouped `bySeverity`; a thread with no round says `noRound: true`; an unreadable thread is a row with its reason, never a skip; an unknown severity string is kept verbatim in `bySeverity`, never dropped; the queue holds only REVISE-latest and no-round threads, oldest-PR first, each entry's `wait` naming the head sha with no verdict (or `'no round posted'`); `totals`; determinism under input shuffle (byte-identical). RED (`ERR_MODULE_NOT_FOUND`).
- [x] T2b. `ui/lib/review-timeline.mjs`: pure, no DOM, no IO — `{ok, value:{threads, queue, totals}}` or the failed section's reason (whichever of `prs`/`reviews` failed first). Threads sorted by PR number for determinism regardless of input order. GREEN 12+/12+. Mutation: the queue's REVISE filter loosened to include APPROVE-latest threads → the queue-membership test red; reverted.
- [x] T3a. `ui/lib/drawer-model.test.mjs` extended: a review entry's `children` carry one row per finding (`severity — id`, the excerpt and `cites` in `detail`), inheriting the round's own `source` (~~no per-finding anchor exists in this provider, D14~~ **CORRECTED, same session**: a per-finding anchor DOES exist when the verdict carried `file`/`line` — D14 is about a per-ROUND forge-comment anchor the provider drops, not a finding's own; a finding now uses its own `{path,line}` source when present, the round's source only as fallback); the detail's finding count reads `findingCount`, not `findings.length`. RED.
- [x] T3b. `ui/lib/drawer-model.mjs`: `reviewEntries` builds one child `entry()` per finding. GREEN. Mutation: `findingCount` swapped for `findings.length` on a malformed verdict (array `[]`, count should stay `null`) → red; reverted.
- [x] T4. `ui/lib/view-model.mjs`: `PLACEHOLDERS.reviews` is now `null`. `ui/static/app.js`: a `reviews` mode router branch renders `buildReviewTimeline`'s output — the queue first ("waiting on a verdict right now"), then one card per PR thread with its rounds oldest first (verdict word + ✓/✕ mark), findings grouped by severity, an unreadable thread as a row with its reason, `'no round posted'` when a thread has no round. `ui/static/app.css`: severity chips and the queue, token-block colours only (new `--severity-*` tokens in both `:root` and the dark media query, alongside the existing `--state-*` set). `views-owned.test.mjs`/`view-model.test.mjs` updated (`reviews` now has real content). RED: `PLACEHOLDERS.reviews` asserted `null` while still the PR-5 sentence → GREEN across `app-source-guard`/`views-owned`/`view-model`/`review-timeline`/`drawer-model`/`tokens`. `renderReviews()` itself: RED/GREEN N/A — no DOM harness (D9), same precedent as R998-2/3/4; verified by trace against `review-timeline.test.mjs`'s already-covered contract.
- [x] T5. `GIT_CONFIG_GLOBAL=/dev/null npm test` and `npm run brain:repo:check` green; counted diff under 400.
- [x] T6. `npm run memory:save -- "<title>" "<content>" --issue 998 --type decision`; stage only the record and `.memory/index.jsonl`.
