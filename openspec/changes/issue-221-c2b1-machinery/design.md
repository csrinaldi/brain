...
## Decision 3 — scrub re-point: new reader, same scanner

A records-file reader (read `.jsonl`, split lines) replaces `scrubChunkFile`'s gunzip step and feeds
the same `scanTextForSecrets`. `share`'s scrub seam swaps from `scrubChunkFile` (over materialized
chunks) to the candidate-record scan (Decision 1). Fail-closed + no bypass are preserved; the
config allowlist path (C1b) is unchanged.
